  ...
  class Cli
    ...
    def run
      Trollop.educate unless set_host? || key? || database? || tmp_disk? || log_disk? ||
                             uninstall_ipa? || install_ipa? || certs? || extauth_opts? || set_server_state?
      if set_host?
    ...
    def set_server_state
      ...
      case options[:server]
      when :status
        service_running ? "running" : "not running"
        say "EvmServed is " + service.running?
      when :start
        service.start unless service_running
      when :stop
        service.stop if service_running
      when :restart
        say "EvmServed is restarting"
        service.restart
      else
        say "wrong server state"
      end
    end
